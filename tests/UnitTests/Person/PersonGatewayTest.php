...
class PersonGatewayTest extends \PHPUnit\Framework\TestCase
{
    protected $procob;

    public function setUp()
    {
        $this->procob = $this->createMock(Procob::class);

        $this->procob
            ->method('persons')
            ->willReturn($this->getPersonsGateway())
        ;
    }
     ...
     * @test
     */
    public function mustReturnPersonObject()
    {
        $cpf = '99999999999';
        ...
        $this->assertEquals($cpf, $person->getDocument());
    }

    public function getPersonsGateway()
    {
        $gateway = $this->createMock(PersonGateway::class);

        $gateway->method('findByCpf')
